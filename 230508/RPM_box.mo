...
equation
  ...
  if (Gear_input == 1) then
    RPM_output = first_dan*sum_RPM*Gear_ratio;
  elseif (Gear_input == 2) then
    RPM_output = second_dan*sum_RPM*Gear_ratio;
  elseif (Gear_input == 3) then
    RPM_output = third_dan*sum_RPM*Gear_ratio;
  elseif (Gear_input == 4) then
    RPM_output = forth_dan*sum_RPM*Gear_ratio;
  elseif (Gear_input == 5) then
    RPM_output = fifth_dan*sum_RPM*Gear_ratio;
  elseif (Gear_input == 6) then
    RPM_output = sixth_dan*sum_RPM*Gear_ratio;
  elseif (Gear_input == 7) then
    RPM_output = back_dan*sum_RPM*Gear_ratio;
  else
    RPM_output = 0;
  end if
